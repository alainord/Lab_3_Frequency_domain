...
F = fft2(I);
Fshift = fftshift(F); % center zero frequency
S = log(1 + abs(Fshift));
...
D0 = 40; % cutoff radius
H_ideal_LP = double(D <= D0); % ideal circular low-pass

sigma = 20;
H_gauss_LP = exp(-(D.^2) / (2*sigma^2));

figure;
subplot(1,2,1); imshow(fftshift(H_ideal_LP),[]); title('Ideal LP filter');
subplot(1,2,2); imshow(fftshift(H_gauss_LP),[]); title('Gaussian LP filter');
